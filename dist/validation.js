...
export class TxValidator {
     ...
     */
    async validateTx(tx, currentBlockGasUsed = 0n) {
        try {
     ...
     */
    validateGas(tx, currentBlockGasUsed) {
    ...
    validateTransactionType(tx) {
        switch (tx.type) {
            case "transfer":
                return this.validateTransferTx(tx);
            case "post":
                return this.validatePostTx(tx);
            case "rep":
                return this.validateRepTx(tx);
            case "deploy":
                return this.validateDeployTx(tx);
            case "call":
                return this.validateCallTx(tx);
            default:
                return { valid: false, error: `Unknown transaction type: ${tx.type}` };
        }
    }
     ...
     */
    async validateSignedTx(stx, chainId) {
        try {
...
export class RateLimiter {
    ...
    windowMs;
    txCounts = new Map();
    constructor(maxTxPerMinute = 60, windowMs = 60000 // 1 minute
    ) {
