...
async function startNode() {
    try {
        ...
        const p2p = new P2P(async (tx) => {
            try {
                await chain.addTx(tx);
            }
            catch (error) {
                errorHandler.logError(error, 'P2P Transaction Processing', { txHash: tx.hash, from: tx.tx.from });
                throw new TransactionError(`Failed to process transaction: ${error.message}`, tx.hash);
            }
        }, (b) => {
            (async () => {
                try {
                    ...
                    errorHandler.logError(error, 'Block Import', { blockHeight: b.header.height, blockHash: b.hash });
                    throw new ConsensusError(`Failed to import block: ${error.message}`, b.header.height);
                }
            })();
        });
        ...
        shutdownHandler.onShutdown(async () => {
            ...
            try {
                ...
                fs.writeFileSync(mempoolPath, JSON.stringify(chain.mempool, (key, value) => typeof value === 'bigint' ? value.toString() : value, 2));
            }
            catch (error) {
        ...
        if (cfg.isLeader) {
            ...
            const blockInterval = setInterval(async () => {
                try {
                    ...
                    errorHandler.logError(error, 'Block Production', { mempool_size: chain.mempool.length });
                }
            }, cfg.blockTimeMs);
            ...
            if (cfg.leaderWsURL) {
                ...
                }, 'Leader Connection', 5, 2000);
            }
        }
        // Start the API server with enhanced error handling
        startApi(cfg.apiPort, {
            submitTx: async (stx) => {
                await errorHandler.handleError(() => chain.addTx(stx), 'Transaction Submission', 1, // No retries for transaction submission
                0);
            },
            ...
            getContractStorage: (address, key) => chain.getContractStorage(address, key),
            getReceipt: (txHash) => chain.getReceipt(txHash),
        ...
        if (err instanceof Error) {
            ...
            errorHandler.logError(err, 'Node Initialization');
        }
        else {
